...
- ~~**Group C — Option B top-of-funnel redesign**~~ **DEPRIORITIZED 2026-05-29.** Still architecturally clean, but per the 3.4 pivot below it's no longer the next-action candidate. May resurface if doc-switching/intent research surfaces a top-of-funnel redesign as the principled solution — in which case Group C and the intent-understanding work converge into one redesign rather than two.
- **Group B — indexing changes** (B8 section_path — landed & pushed; B10 per-doc summary — landed & pushed 2026-08-05; B9 Contextual Retrieval — committed locally 2026-05-26, **PARKED on push 2026-08-05** via a revert commit that keeps B9's code out of prod until Stage 3A reactivates it). B9's local backfill (5092 chunks across all indexed TAs) ran in ~3h 51min. The audit's "B9 unlocks the deferred B17 reranker swap + B19 supplementary-teaching deletion" framing partially held — B19 is now KILLED (see 3.4 below), and B9's reactivation is bundled with the Stage 3A reranker work.
- ~~**`hybrid_doc_search` refactor → summary-cosine + LLM tiebreaker** (unlocked by B10, ~300 lines deletable).~~ **ATTEMPTED 2026-05-26, ROLLED BACK** — regressed 92-row eval from 75.5% → 61.0% hit@5 (−14.5pp). The Stage 5 short-circuit's exact-token matching is load-bearing for Maize's numeric/structural query distribution ("pset 2", "lecture 5", "extra problems II"); summary-cosine dilutes those signals. Anthropic's published 49% lift was on text-corpus benchmarks without specialized numeric routing; doesn't transfer to Maize. Full diagnosis: `attached_assets/maize-hybrid-doc-search-refactor-plan.md` ("Result — did not ship"). **B10's `Document.summary_embedding` column remains useful for explainability/debugging but NOT as a primary doc-routing signal.**
- ~~**B19 supplementary-teaching deletion** (~140 lines, unlocked by B9).~~ **KILLED 2026-05-29 on product grounds.** B19 retrieves extra concept-related chunks specifically to give the LLM pedagogical ammo grounded in the *professor's own materials*. The audit's reasoning ("LLM can substitute its training-data knowledge once chunks self-describe via B9") directly contradicts Maize's product positioning — methodology preservation IS the USP, not an implementation detail. Removing B19 would mean the LLM falls back to generic training knowledge for concept reinforcement, which is exactly what the product is supposed to NOT do. Audit's product context was incomplete on this point. **B19 stays; the "deletable cruft" framing was wrong.**
...
- **Multi-TA eval expansion** — ✓ DONE 2026-05-25. 92-row body across 3 TAs (ECON 64 + EC 112 9 + MGT 410 19); F1/F2/G1/G2 buckets + warm-cache replay mode.

### 3.4 Post-comprehensive-pass pivot — surgical focus on doc-switching / intent understanding (2026-05-29)

**The shift.** The audit was a comprehensive pass: map the system, audit each step against 2024-2026 literature, propose ranked Phase 1/2/3 work. Phase 1 (Group A cleanup) and Phase 3 (Group B indexing trilogy) shipped. Phase 2 (Group C top-of-funnel + retrieval-side cleanups) is where the comprehensive plan ran into Maize-specific reality:

| Item | Audit said | Reality |
|---|---|---|
| `hybrid_doc_search` refactor | "Largest single retrieval simplification, ~300 lines deletable" | Empirically wrong: regressed 92-row eval −14.5pp; structural-identifier queries need exact-token matching that summary-cosine dilutes |
| B19 supplementary-teaching deletion | "Cleanest after B9 lands" | Wrong on product grounds: removing course-grounded pedagogical context contradicts the methodology-preservation USP |
| Group C top-of-funnel redesign | "Largest single retrieval-side change" | Possibly correct, but the audit's prediction track record now warrants explicit evidence before another 1-2 session bet |

**The principle.** Comprehensive-first was right at the start — the system had under-understood failure modes and we needed the map. Post-audit + post-misses, the situation inverts: we now have measurement infrastructure (the 92-row eval) and clearer signal about where real failures live, so *surgical intervention on the highest-evidence failure mode is the right move* — and is consistent with the [feedback_principled_redesigns](../.claude/projects/-Users-simonkleffner-Desktop-Maize-TA-Master-App-Dev-Maize-Blueprint-V2/memory/feedback_principled_redesigns.md) principle (data → research → validate → implement → verify).

**The evidence for the pivot target.** Failure-rate-per-bucket analysis on the post-B9 scorecard:

| Bucket | n total | prod_log | composition | post-B9 hit@5 | Reliable for prod failure rate? |
|---|---:|---:|---|---:|---|
| A (Lab vs PS) | 6 | 3 | 50% prod / 50% synthetic | 100% | No — heavily synthetic |
| B (Roman cross-doc) | 5 | 1 | 20% prod / 80% synthetic | 80% | No — heavily synthetic |
| C (lookalike) | 7 | 4 | 57% prod / 43% synthetic | 71% | Mixed, weak signal |
| D (problem-vs-solutions) | 5 (in-corpus) | 5 | 62% prod / 38% synthetic | 60% | Mixed, small-n |
| **E (cache anchoring / doc-switching)** | **22** | **19** | **86% prod / 14% synthetic** | **73%** | **YES — essentially prod data** |
| F1/F2/G1/G2 | 0-2 | all prod | small-n outliers | varies | Unusable for rates |
| Working cases | 40 | 12 | 30% prod / 70% synthetic_working | 78% | No — heavily synthetic-easy |

**E is the only well-measured bucket on real prod data, and it has the highest reliable failure rate (~20-25% on prod-only after bounding the 3 synthetic rows).** This aligns with direct prod experience that doc-switching / context drift / intent misclassification is the dominant complaint pattern (see `attached_assets/Maize QA Master [PROD] - Context drift.csv`, `... - Topic switching.csv`, `... - pset switch.csv`).

**Reframing E.** "Cache anchoring" describes the symptom (session doc cache carries the wrong doc into a new-intent turn). The underlying disease is **intent misunderstanding** — `contextualize_query` classifies "continuation vs pivot" incorrectly on pronoun-prefixed or implicit doc switches, the cache stays warm with stale state, and retrieval surfaces stale-doc chunks. Fixing the cache is patching the symptom; fixing intent classification is principled. The latter is the focal area.

**Next action.** Research-first per the principled-redesigns workflow:
1. **Broad**: how do production conversational RAG systems architect user-intent processing? Single structured tool call vs multi-step pipelines; intent representation and propagation; evaluation methodologies.
2. **Narrow**: how do production RAG systems handle doc/resource switching specifically? Pivot detection signals; cache invalidation policies; explicit-doc-switch recognition patterns.

Both via the `deep-research` skill with adversarial verification + transferability checks (see 3.3 "audit-miss lessons"). Source bar: 2025-2026 production write-ups from credible practitioners (Vectara, Cohere, LangChain, LlamaIndex, Anthropic, OpenAI engineering blogs); peer-reviewed papers; engineering write-ups from teams actually running RAG at scale.

**Decision boundary**: research findings determine whether next move is (a) principled redesign of the contextualizer + session state (broad pattern application), (b) tactical pivot-detection intervention bolted into today's contextualizer (narrow patch), or (c) something else surfaced by the research. No implementation begins until the research is done and a plan is reviewed.
...
| 2026-05-25 | B10 indexing-only ship: Document.summary + summary_embedding columns + summarize_doc LLM call wired into indexing pipeline + ECON S1117 backfill (68/68 docs). Retrieval-side activation (hybrid_doc_search → summary-cosine + LLM tiebreaker, ~300 lines deletable) deferred. | commit `9eb81b5` |
| 2026-05-25 | Eval framework expansion: 92-row body across 3 TAs (ECON S1117 64 + EC 112 9 + MGT 410 19); F1/F2/G1/G2 failure types added; warm-cache mode default ON; cold-cache opt-in flag. | commit `2a456a2` |
| 2026-05-26 | B9 indexing-only ship (LOCAL ONLY, later reverted on push — see 2026-08-05): Anthropic Contextual Retrieval — DocumentChunk.contextual_prefix column + generate_contextual_prefix LLM call wired into indexing pipeline + cross-TA backfill (5092 chunks across ECON 2195 + MGT 410 883 + EC 112 514 + other indexed TAs). 92-row eval pre→post comparison: overall hit@5 75.2%→75.5% (flat, within noise); C bucket +14pp, G2 +50pp on small n; reranker contribution shifted from net-negative to net-positive on B/C/G2 (audit's predicted "self-describing chunks help reranker" effect, indirect via changed initial retrieval — the reranker does NOT read contextual_prefix in the current retriever code). See `attached_assets/maize-b9-contextual-retrieval-plan.md`. | commit `8cc357e` |
| 2026-05-26 | **NEGATIVE RESULT**: `hybrid_doc_search` → summary-cosine + LLM tiebreaker refactor (audit's "largest single retrieval simplification") attempted and rolled back. Implementation completed (`summary_doc_search` + tiebreaker + flag); 92-row eval regressed overall hit@5 75.5%→61.0% (−14.5pp). Failure mode: numeric/structural queries lose exact-token matching that BM25 + filename + Stage 5 short-circuit provide; dense summary embeddings dilute the signal. Anthropic's 49% lift figure doesn't transfer to Maize's query distribution. Code reverted; learning preserved in `attached_assets/maize-hybrid-doc-search-refactor-plan.md` "Result — did not ship" + defensive comment above `hybrid_doc_search` in `src/retriever.py`. | commit `6a0975a` |
| 2026-05-29 | **STRATEGIC PIVOT**: comprehensive-plan execution → surgical focus on doc-switching / intent understanding. Triggers: (1) hybrid_doc_search refactor empirically wrong (audit miss); (2) B19 deletion killed on product-positioning grounds (methodology preservation IS the USP); (3) failure-rate-per-bucket analysis on 92-row eval identifies E (cache anchoring / doc-switching) as the only well-measured bucket on prod data and the highest reliable failure rate (~20-25% on prod-only slice). Reframed: E is symptomatic; underlying disease is intent classification in `contextualize_query`. Next move: research-first via `deep-research` skill — (a) broad: production conversational-RAG intent processing architectures; (b) narrow: doc/resource switching patterns specifically. No implementation begins until research + reviewed plan. See section 3.4. | commit `11c9e1f` |
| 2026-08-05 | **PILOT-FEEDBACK PIVOT + DEPLOYMENT HYGIENE**: summer pilot surfaced latency (25-70s per turn) as the adoption-erosion driver. Priorities re-ordered: latency-optimization plan (existing `attached_assets/maize-latency-optimization-plan.md`) takes over the immediate arc; intent-classification LangGraph work deferred. Deployment hygiene: local dev queue cleared — pushed B10 (`9eb81b5`), eval expansion (`2a456a2`), negative-result (`6a0975a`), pivot audit-doc (`11c9e1f`), and this entry. B9 (`8cc357e`) was PARKED via a git revert (non-destructive) rather than pushed — reason: fresh reranker research is 2+ months stale, B9's payoff only materializes when combined with Stage 3A reranker swap + a follow-up code change to include the prefix in reranker input. B9's code remains in git history at commit `8cc357e`; reactivation is a git-revert-of-revert when Stage 3A is ready. | _this commit_ + subsequent push |
| _TBD_ | Doc-switching/intent research findings + redesign plan | _TBD_ (deferred behind latency work) |
