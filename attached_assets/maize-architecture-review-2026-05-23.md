...
- **Group B — indexing changes** (B8 section_path — landed; B10 per-doc summary — landed; B9 Contextual Retrieval — landed locally 2026-05-26). All three Group B indexing changes shipped; their retrieval-side activations are deferred behind flags pending D12 multi-TA data + A/B testing. B9 was the heaviest of the three (full re-index of every chunk, ~$5-10 prod cost when rolled out); local-only cross-TA backfill (5092 chunks across all indexed TAs) ran in ~3h 51min. B9 unlocked the deferred B17 reranker swap + B19 supplementary-teaching deletion (~140 lines) since chunks are now self-describing — but B19 is now KILLED (see 3.4 below).
...
| 2026-05-26 | B9 indexing-only ship: Anthropic Contextual Retrieval — DocumentChunk.contextual_prefix column + generate_contextual_prefix LLM call wired into indexing pipeline + cross-TA backfill (5092 chunks across ECON 2195 + MGT 410 883 + EC 112 514 + other indexed TAs). 92-row eval pre→post comparison: overall hit@5 75.2%→75.5% (flat, within noise); C bucket +14pp, G2 +50pp on small n; reranker contribution shifted from net-negative to net-positive on B/C/G2 (audit's predicted "self-describing chunks help reranker" effect). Retrieval-side activation (B17 reranker swap + B19 supplementary-teaching deletion) unlocked but deferred. See `attached_assets/maize-b9-contextual-retrieval-plan.md`. | commit `8cc357e` |
